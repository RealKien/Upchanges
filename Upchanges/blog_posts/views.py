...
@blog_posts.route('/<blog_validated_id>/add_project', methods=['GET', 'POST'])
@login_required
def add_project(blog_validated_id):
    ...
    if form4.validate_on_submit():
        ...
        blog_project_validated = BlogProject(project_name=form4.project_name.data,
                                             project_image=add_project_pic(form4.project_image.data, form4.project_name.data + str(current_user.id)),
...
##########################################
    ...
    projects1 = BlogProject.query.filter(BlogProject.blog_post_id.ilike(blog_validated_id)).order_by(BlogProject.date.desc())
...
def blog_idea_update(blog_validated_id,blog_idea_id):
    ...
    return render_template('blog_idea_update.html', blog_validated_id=blog_validated_id, form2=form2,
                           ...
                           projects=projects, projects_num=projects_num)





@blog_posts.route('/<int:blog_validated_id>/<blog_project_id>/update_project', methods=['GET', 'POST'])
@login_required
def blog_project_update(blog_validated_id,blog_project_id):
    ...
    if form4.validate_on_submit():
        ...
        if form4.project_image.data:
            blog_project_update.project_image = add_project_pic(form4.project_image.data, form4.project_name.data + str(current_user.id))
            # blog_update.blog_image=blog_image
        ...
        return redirect(url_for('blog_posts.blog_view',blog_validated_id=blog_project_update.blog_post_id, project_image=url_for('static', filename='project_pics/' + blog_project_update.project_image)))
...
@blog_posts.route('/<int:blog_validated_id>/<blog_project_id>/delete_project', methods=['GET', 'POST'])
@login_required
