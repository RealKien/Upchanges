...
#View blog post
@vn_blog_posts.route('/vietnam/<int:blog_validated_id>', methods=['GET', 'POST']) #int makes a string a number so that BlogPost.query doesn't show error(because data can only stored as number)
def blog_view(blog_validated_id):
    ...
    return render_template('blog_view.html', problem_name=blog_view.problem_name,
                           ...
                           blog_image=blog_view.blog_image,
                           post=blog_view,
                           problem_type= blog_view.problem_type, country=country, total_num=total_num,ideas=ideas, comment_blogs=comment_blogs,
                           blog_idea_check=blog_idea_check, blog_info_check=blog_info_check, form2=form2, blog_validated_id=blog_validated_id, form3=form3)
